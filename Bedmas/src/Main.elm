...
--     httpspackage.elm-lang.orgpackageselmcorelatestDict
  ...
  case expr0 of
      Const float - -- probably assume positive
        let
            width = if float  10 then charWidth
                else if float  100 then 2charWidth
                     else if float  1000 then 3charWidth
                          else 4charWidth
        in
            ( group 
              ( if highlight == CConst
      ...
      Plus expr expr2 -
        ...
        in
          ( group ( hl ++
                [
                  text (  txtFmt  centered  filled orange  move (-0.125charWidth - leftWidth, 0)
                , left  move (-0.5  (0.25charWidth + leftWidth),0)
                , text +  txtFmt  centered  filled orange
                , circle 3  filled (rgba 0 0 0 0.1)  move (0,1)  notifyTap (Tap  mkClickable CPlus)
                , right  move (0.5  (0.25charWidth+rightWidth),0)
                , text )  txtFmt  centered  filled orange  move (0.125charWidth + rightWidth,0)
                -- debug , rect (1charWidth + leftWidth + rightWidth) 1  filled orange
                ]
            )  move ( 0.5  (leftWidth - rightWidth), 0)
          , 1charWidth + leftWidth + rightWidth )
      ...
      Mult expr expr2 -
        ...
        in
          ( group ( hl ++
            [
              text (  txtFmt  centered  filled black  move (-0.125charWidth - leftWidth, 0)
            , left  move (-0.5  (0.25charWidth + leftWidth),0)
            , text   txtFmt  centered  filled black
            , circle 3  filled (rgba 0 0 0 0.1)  move (0,1)  notifyTap (Tap  mkClickable CMult)
            , right  move (0.5  (0.25charWidth+rightWidth),0)
            , text )  txtFmt  centered  filled black  move (0.125charWidth + rightWidth,0)
            -- debug , rect (1charWidth + leftWidth + rightWidth) 1  filled red
            ]
            )  move ( 0.5  (leftWidth - rightWidth), 0)
          , 1charWidth + leftWidth + rightWidth )
