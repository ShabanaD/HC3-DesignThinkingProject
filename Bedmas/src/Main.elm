...
myShapes model =
  ...
  , group [
    roundedRect 40 10 6 |> filled (rgb 150 133 182) |> move ( -70, -60 )
    , text "Hint" |> size 7 |> filled black |> move (-75, -62)
    ] |> notifyTap (GiveHint)
  , text (model.hint) |> size 7 |> filled black |> move (-25, -50)
  , group [
    roundedRect 70 10 6 |> filled (rgb 150 133 182) |> move ( 20, -60 )
...
update msg model =
    case msg of
        ...
        Tap clickable ->
          { model | highlight = clickable, hint = "" }
        -- CLICKFUNCTION State -> case State of
        --         state1 -> {model | expr = }
        GiveHint ->
          case (model.state, model.highlight) of
            (Ex2,CNotHere) -> { model | hint = "Try clicking on an operation" }
            (Ex2,CSubt) -> { model | hint = "Look inside the brackets :)" }
            otherwise -> { model | hint = "" }
        SwitchEx ->
          case (model.state) of
            (Ex2) -> { model | state = Ex1, expr = example1}
            (Ex1) -> { model | state = Ex2, expr = example2}
